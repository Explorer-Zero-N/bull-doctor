...
fn write_shared_icon_assets() {
    ...
    let png_path = assets_dir.join("bull-doctor.png");
    let rgba = icon_render::render_icon_rgba(512);
    write_png(&png_path, 512, &rgba).expect("write bull-doctor.png");
}
...
fn write_windows_icon_resources() {
    ...
    let assets_dir = PathBuf::from("assets");
    std::fs::write(assets_dir.join("bull-doctor.ico"), &ico_bytes).ok();

    let mut res = winres::WindowsResource::new();
    res.set_icon(ico_out.to_str().expect("icon path utf-8"));
    res.set("ProductName", "Bull Doctor");
    res.set("FileDescription", "Bull Doctor");
    res.set("LegalCopyright", "Bull Doctor");
