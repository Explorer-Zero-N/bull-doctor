...
/// Supports tool-prefixed paths like `/hermes/v1/chat/completions`.
pub fn classify_compressible_path(path: &str) -> Option<CompressibleEndpoint> {
    // 先精确匹配
    match path {
        "/v1/messages" => return Some(CompressibleEndpoint::AnthropicMessages),
        "/v1/chat/completions" => return Some(CompressibleEndpoint::OpenAiChatCompletions),
        "/v1/responses" => return Some(CompressibleEndpoint::OpenAiResponses),
        _ => {}
    }
    // 再尝试去掉工具前缀匹配（如 /hermes/v1/chat/completions → /v1/chat/completions）
    if let Some(rest) = strip_tool_prefix(path) {
        return classify_compressible_path(rest);
    }
    None
}

/// 去掉已知的工具路径前缀。
fn strip_tool_prefix(path: &str) -> Option<&str> {
    const PREFIXES: &[&str] = &[
        "/claude-desktop",
        "/openclaw",
        "/hermes",
