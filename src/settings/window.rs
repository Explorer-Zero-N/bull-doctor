...
pub fn close_settings_window(slot: &mut Option<SettingsWindow>, window_id: WindowId) -> bool {
    ...
    // 保存窗口大小
    save_current_window_size(&settings.window);
    slot.take();
    SETTINGS_OPEN.store(false, Ordering::SeqCst);
    true
}

fn save_current_window_size(window: &Window) {
    let size = window.inner_size();
    if let Ok(mut app) = AppConfig::load() {
        app.settings_window_width = Some(size.width);
        app.settings_window_height = Some(size.height);
        let _ = app.save();
    }
}
...
fn create_settings_window<T>(
    ...
    let w = app.settings_window_width.unwrap_or(580);
    let h = app.settings_window_height.unwrap_or(620);

    let window = WindowBuilder::new()
        .with_title("Bull Doctor · 设置")
        .with_window_icon(Some(crate::icon::window_icon()))
        .with_inner_size(tao::dpi::LogicalSize::new(w as f64, h as f64))
        .with_min_inner_size(tao::dpi::LogicalSize::new(440.0, 480.0))
        .with_resizable(true)
        .build(elwt)?;
