...
pub async fn run_with_proxy(app: AppConfig) -> anyhow::Result<()> {
    ...
    // Compression 自动启动
    if app.compress.auto_start {
        let compress_port = app.compress.port;
        let helper_port = app.proxy.port;
        let hr_handle = proxy.compress_process.clone();
    ...
    let settings_slot: Arc<Mutex<Option<settings::SettingsWindow>>> =
        Arc::new(Mutex::new(None));
    let logs_slot: Arc<Mutex<Option<logs::LogsWindow>>> = Arc::new(Mutex::new(None));
    ...
    let model = app
        ...
        .map(|p| p.name.as_str())
        .unwrap_or("?");
    println!("✅ Bull Doctor · {} · {}", model, app.proxy_base_url());
            ...
            *control_flow = ControlFlow::Wait;
            ...
            match event {
                ...
                Event::UserEvent(TrayUserEvent::OpenSettings) => {
                    let mut slot = ctx_for_loop.settings.lock().unwrap();
                    if slot.is_some() {
                ...
                Event::UserEvent(TrayUserEvent::OpenLogs) => {
                    let mut slot = ctx_for_loop.logs.lock().unwrap();
                    if slot.is_some() {
                ...
                Event::UserEvent(TrayUserEvent::OpenStats) => {
                    let url = format!("http://{}:{}/admin/stats",
                        ctx_for_loop.rt.block_on(async {
                            ctx_for_loop.config.read().await.proxy.host.clone()
                        }),
                        ctx_for_loop.rt.block_on(async {
                            ctx_for_loop.config.read().await.proxy.port
                        }));
                ...
                Event::WindowEvent {
                    ...
                    let mut settings_slot = ctx_for_loop.settings.lock().unwrap();
                    if settings::close_settings_window(&mut settings_slot, window_id) {
                        return;
                    }
                    let mut logs_slot = ctx_for_loop.logs.lock().unwrap();
...
struct TrayContext {
    ...
    settings: Arc<Mutex<Option<settings::SettingsWindow>>>,
    logs: Arc<Mutex<Option<logs::LogsWindow>>>,
...
fn handle_menu_click(ctx: &Arc<TrayContext>, id: &str) {
    if id == "quit" {
        ...
        ctx.settings.lock().unwrap().take();
        ctx.logs.lock().unwrap().take();
        std::process::exit(0);
    }
    if id == "settings" {
        let _ = ctx.loop_proxy.send_event(TrayUserEvent::OpenSettings);
    ...
    if id == "open_claude" {
        let _ = actions::open_claude_dir();
        return;
    }
    if id == "resync" {
        spawn_tray_task(ctx, async move |w| {
            if let Err(err) = actions::resync_claude(&w.config, &w.proxy).await {
    ...
    if id == "reload_config" {
        ...
        spawn_tray_task(ctx, async move |_w| {
            ...
            let url = format!("http://{}:{}/admin/reload", crate::config::DEFAULT_HOST, crate::config::DEFAULT_PORT);
            match client.post(url).timeout(std::time::Duration::from_secs(5)).send().await {
                Ok(resp) => {
                    if resp.status().is_success() {
                        ...
                        tracing::error!("配置重新加载失败: {}", resp.status());
                    }
                }
                Err(e) => {
                    tracing::error!("重新加载配置请求失败: {e:#}");
                }
            }
        });
        return;
    ...
    if id == "stats" {
        let _ = ctx.loop_proxy.send_event(TrayUserEvent::OpenStats);
        return;
    }
    ...
    if id == "kill_reset_claude" {
        spawn_tray_task(ctx, async move |_w| {
            if let Err(err) = actions::kill_claude_and_reset_defaults().await {
    ...
    Fut: std::future::Future<Output = ()> + Send + 'static,
{
    ...
    rt.spawn(async move {
        task(worker).await;
        // 先立即刷新托盘 UI（菜单和提示文字），让用户立刻看到切换结果
        let _ = loop_proxy.send_event(TrayUserEvent::RefreshUi);
        // 再触发连接检测
        let _ = loop_proxy.send_event(TrayUserEvent::CheckHealth);
    });
...
fn start_health_check(ctx: &Arc<TrayContext>) {
    // 先从磁盘重新加载配置，确保托盘反映最新的保存状态
    if let Ok(app) = AppConfig::load() {
        ctx.rt.block_on(async {
            *ctx.config.write().await = app;
        });
    }
...
fn refresh_tray_ui(ctx: &Arc<TrayContext>) {
    ...
    if let Ok(menu) = build_menu(&app, &health) {
        ctx.tray.set_menu(Some(Box::new(menu)));
        let tip = tooltip_text(&app, &health);
        if let Err(e) = ctx.tray.set_tooltip(Some(tip.as_str())) {
            tracing::warn!("托盘提示更新失败: {e}");
        }
    }
}
...
fn tooltip_text(app: &AppConfig, health: &ProviderHealth) -> String {
    ...
    let model = provider.map(|p| p.default_model.as_str()).unwrap_or("");
    ...
    if !active_key_configured(app) {
        return format!("Bull Doctor — {name}{model_suffix}（需设置 API Key）");
    }
    if health.provider_id == app.active
        && matches!(health.connection, ConnectionStatus::Failed(_))
    {
        return format!("Bull Doctor — {name}{model_suffix}（连接失败）");
    }
    format!("Bull Doctor — {name}{model_suffix}（运行中）")
}
...
fn build_menu(app: &AppConfig, health: &ProviderHealth) -> anyhow::Result<Menu> {
    ...
    for preset in crate::provider::list_presets(app) {
        ...
        if !preset.custom_models.is_empty() {
            // 使用 API 获取的模型列表替换内置模型
            for entry in &preset.custom_models {
                let is_active =
                    preset.id == app.active && preset.default_model == entry.id;
                let check = if is_active { "✓ " } else { "" };
                let label = format!("{}{}", check, entry.display_name);
                provider_sub.append(&MenuItem::with_id(
                    format!("use:{}:{}", preset.id, entry.id),
                    label,
                    true,
                    None,
                ))?;
        ...
        // 如果当前激活模型不在已展示的列表中，作为自定义条目追加
        if !preset.default_model.is_empty() {
            let in_list = if !preset.custom_models.is_empty() {
                preset.custom_models.iter().any(|e| e.id == preset.default_model)
            } else {
                crate::provider::models::find_model(&preset.id, &preset.default_model).is_some()
            };
            if !in_list {
                let is_active = preset.id == app.active;
                let check = if is_active { "✓ " } else { "" };
                let custom_label = format!("{}{}（自定义）", check, preset.default_model);
                provider_sub.append(&MenuItem::with_id(
                    format!("use:{}:{}", preset.id, preset.default_model),
                    custom_label,
                    true,
                    None,
                ))?;
    ...
    menu.append(&MenuItem::with_id(
        "kill_reset_claude",
        "重置 Claude Code 为默认设置",
        true,
        None,
    ))?;

    menu.append(&PredefinedMenuItem::separator())?;
    menu.append(&MenuItem::with_id("quit", "退出 Bull Doctor", true, None))?;
    Ok(menu)
}
