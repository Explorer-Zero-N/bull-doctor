...
$ExePath = Join-Path $ProjectRoot 'target\release\bull-doctor.exe'
$ReadmeSrc = Join-Path $ProjectRoot 'installer\USAGE-zh-CN.txt'
$IssPath = Join-Path $ProjectRoot 'installer\BullDoctor.iss'
...
Write-Host "Bull Doctor v$Version"
...
if (-not $SetupOnly) {
    ...
    $staging = Join-Path $env:TEMP "bull-doctor-pack-$Version"
    if (Test-Path $staging) { Remove-Item $staging -Recurse -Force }
    New-Item -ItemType Directory -Force -Path $staging | Out-Null
    Copy-Item $ExePath (Join-Path $staging 'bull-doctor.exe') -Force
    if (-not (Test-Path $ReadmeSrc)) {
        throw "Missing $ReadmeSrc"
    }
    Copy-Item $ReadmeSrc (Join-Path $staging 'USAGE-zh-CN.txt') -Force

    $zipPath = Join-Path $DistDir "BullDoctor-$Version-win64.zip"
...
"@
    ...
    $setupPath = Join-Path $DistDir "BullDoctor-$Version-Setup.exe"
